...
class l {
	...
	attachPlugins(t) {
		...
		for (const [s, o] of Object.entries(t || {}))
			!1 === this.options[s] ||
				this.plugins[s] ||
				((this.options[s] = e({}, o.defaults || {}, this.options[s])), (i[s] = new o(this)));
		...
		const t = new h(this.$container, {
			start: (e, i) => {
				...
				if (!t.currentPointers.length) {
					if (
						-1 !== ['BUTTON', 'TEXTAREA', 'OPTION', 'INPUT', 'SELECT', 'VIDEO'].indexOf(o.nodeName)
					)
						return !1;
					if (
						this.option('textSelection') &&
...
class y extends l {
	...
	updateMetrics() {
		...
		if (null === u) {
			...
			(u =
				null !== t ? this.findPageForSlide(t) : parseInt(this.option('initialPage', 0), 10) || 0),
	...
	setPage(t, e) {
		...
		if (((t = ((s % a) + a) % a), this.option('infiniteX', this.option('infinite')) && r > h)) {
			...
			(t = s = Math.max(0, Math.min(s, a - 1))), (i = this.pages.length ? this.pages[t].left : 0);
		return (
			(this.page = t),
			(this.pageIndex = s),
			null !== o &&
...
class P {
		...
		if (!i || 'string' == typeof i) {
			if (
				...
				(o = i.match(
					...
					? ((t.src = `//maps.google.${o[1]}/?ll=${(o[2]
							? o[2] + '&z=' + Math.floor(o[3]) + (o[4] ? o[4].replace(/^\//, '&') : '')
							: o[4] + ''
					  ).replace(/\?/, '&')}&output=${
			...
			s ||
				('#' === i.charAt(0)
					...
					? ((s = 'html5video'), (t.format = t.format || 'video/' + ('ogv' === o[1] ? 'ogg' : o[1])))
					: i.match(
				...
				('html5video' !== s && 'video' !== s) ||
					...
					t._width && t._height
						...
						: (t.ratio = t.ratio || t.video.ratio || E.video.ratio));
		}
	}
	onReady() {
	...
	loadIframeContent(t) {
		...
		if (
			...
			return (
				i.setAttribute('src', t.src), this.fancybox.setContent(t, i), void this.resizeIframe(t)
			);
		e.showLoading(t);
		...
		(s.style.visibility = 'hidden'),
			...
			(i.onload = () => {
				...
				i.isReady || ((i.isReady = !0), (s = !0)),
					i.src.length &&
						((i.parentNode.style.visibility = ''), this.resizeIframe(t), s && e.revealContent(t));
			}),
...
class R extends l {
	...
	setItems(t) {
		...
		for (const i of t) {
			...
			!o &&
				s &&
				((o = s.currentSrc || s.src), !o && s.dataset && (o = s.dataset.lazySrc || s.dataset.src)),
		...
		for (let t of o) {
			...
			e && (i || o)
				? (a.push(t),
				  ...
				  (t.hasAttribute('autoFocus') || (!n && i && !t.classList.contains('carousel__button'))) &&
						(n = t))
	...
	static fromEvent(t, e = {}) {
		...
		if (
			((a.matches('[data-fancybox-trigger]') || (a = a.closest('[data-fancybox-trigger]'))) &&
				((e.triggerTarget = a), (s = a && a.dataset && a.dataset.fancyboxTrigger)),
			s)
		) {
	...
	static fromOpener(t, i = {}) {
		...
		if ((n && !a && (s = h ? s.filter(t => t.getAttribute(`${r}`) === h) : [n]), !s.length))
			return !1;
		...
		return (
			...
			new R(s, e({}, i, { startIndex: o, $trigger: n })))
		);
	}
	static bind(t, e = {}) {
		function i() {
...
for (const [t, e] of Object.entries(R.Plugins || {}))
